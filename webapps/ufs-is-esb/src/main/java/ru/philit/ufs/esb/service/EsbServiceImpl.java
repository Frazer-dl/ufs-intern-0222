...
import ru.philit.ufs.model.converter.esb.asfs.OverLimitAdapter;
...
public class EsbServiceImpl
  ...
  public void sendRequest(ExternalEntityRequest entityRequest) {
    ...
    try {
      switch (entityRequest.getEntityType()) {
        ...
        case RequestType.OPERATOR_BY_ID:
          if (isRequestDataString(entityRequest)) {
            SrvGetOperatorInfoByIdRq request = OperatorAdapter.requestById(
                (String) entityRequest.getRequestData());
            isEsbCache.putRequest(request.getHeaderInfo().getRqUID(), entityRequest);
            esbClient.sendMessage(pprbConverter.getXml(request));
          }
        ...
        case RequestType.CHECK_OVER_LIMIT:
          if (isCheckOverLimitRequest(entityRequest)) {
            SrvCheckOverLimitRq request = OverLimitAdapter
                .requestOverLimit((Limit) entityRequest.getRequestData());
            isEsbCache.putRequest(request.getHeaderInfo().getRqUID(), entityRequest);
        ...
        case RequestType.CREATE_CASH_ORDER:
          if (isCashOrderRequest(entityRequest)) {
            SrvCreateCashOrderRq request = CashOrderAdapter
                .requestCreateCashOrder((CashOrder) entityRequest.getRequestData());
            isEsbCache.putRequest(request.getHeaderInfo().getRqUID(), entityRequest);
            esbClient.sendMessage(asfsConverter.getXml(request));
          }
          break;
        ...
        case RequestType.GET_WORKPLACE_INFO:
          if (isRequestDataString(entityRequest)) {
            SrvGetWorkPlaceInfoRq request = WorkPlaceAdapter
  ...
  private boolean isCheckOverLimitRequest(ExternalEntityRequest entityRequest) {
    return entityRequest.getRequestData() != null
        && entityRequest.getRequestData() instanceof Limit;
  }
